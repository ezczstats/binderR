...
#Step 5#uncommment the code below (PUT THIS IN THE INPUT CHUNK!!!!!!!)
...
#Step 7# Uncomment the code below... PUT THIS AFTER SELECT INPUT IN THE INPUT CHUNK!! 
...
```{r}
selectInput("nationality",label="Nationality",
            choices=unique(fifa$Nationality),selected=unique(fifa$Nationality)[1],
            multiple=TRUE)
...
renderPlotly({
  ...
  ggplotly(new_df()%>%
    ggplot(aes(x=SprintSpeed,y=Agility))+
    geom_point(aes(color=Nationality,text=Name))+
...
#Step 10 --> change the Column below to a tabset Column by typing {.tabset} next to the word Column Make sure there is a space after Column
...
renderPlotly({
  ...
  ggplotly(new_df()%>%
    ...
    geom_density(aes(color=Nationality))+
    theme_bw()+
    facet_wrap(~Nationality))
